...
def create_groups(apps, schema_editor):
    ...
    player_in_team = apps.get_model('competitions', 'PlayerInTeam')
    ...
    content_type_player_in_team = ContentType.objects.get_for_model(player_in_team)

    team_owner_permissions = Permission.objects.filter(content_type=content_type_team) | Permission.objects.filter(
        content_type=content_type_player) | Permission.objects.filter(content_type=content_type_player_in_team)
    competition_owner_permissions = Permission.objects.filter(
        ...
        content_type=content_type_competition) | Permission.objects.filter(
        content_type=content_type_match) | Permission.objects.filter(content_type=content_type_player_in_team)
